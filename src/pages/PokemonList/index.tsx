...
import { getLastViewedPokemon, type LastViewedPokemon } from '../../services/ultimoVisto';
...
export default function PokemonListScreen() {
 ...
 const [lastViewedPokemon, setLastViewedPokemon] = useState<LastViewedPokemon | null>(null);
 ...
 async function loadMore() {
   if (showOnlyFavorites || isLoadingMore || isInitialLoading || isRefreshing || !hasNextPage) {
     return;
   }
 ...
 async function loadStoredState() {
   try {
     ...
     const [ids, favorites, lastViewed] = await Promise.all([
       getFavoriteIds(),
       getFavoritePokemons(),
     ...
     setLastViewedPokemon(lastViewed);
   } finally {
   ...
   >
     <View style={styles.cardLeft}>
       <Text style={styles.cardName}>
         {item.name} {favoriteIds.includes(item.id) ? '★' : '☆'}
       </Text>
   ...
   <View style={styles.container}>
     ...
     {lastViewedPokemon ? (
       <TouchableOpacity
         ...
         activeOpacity={0.8}
         onPress={() => navigation.navigate('PokemonDetail', { id: lastViewedPokemon.id })}
       >
         <View style={styles.lastViewedHeader}>
           <Text style={styles.lastViewedLabel}>Ultimo visto</Text>
           <Text style={styles.lastViewedId}>#{String(lastViewedPokemon.id).padStart(3, '0')}</Text>
         </View>
         <View style={styles.lastViewedContent}>
           <View style={styles.cardLeft}>
             <Text style={styles.cardName}>
               {lastViewedPokemon.name} {favoriteIds.includes(lastViewedPokemon.id) ? '★' : ''}
             </Text>
             <View style={styles.typeContainer}>
               {lastViewedPokemon.types.map((type) => (
                 <View
                   key={`last-viewed-${lastViewedPokemon.id}-${type}`}
                   style={[styles.typeBadge, { backgroundColor: getTypeColor(type) }]}
                 >
                   <Text style={styles.typeText}>{type}</Text>
                 </View>
           ...
           <Image source={{ uri: lastViewedPokemon.imageUrl }} style={styles.cardImage} />
         </View>
       </TouchableOpacity>
     ) : null}
     ...
     <FlatList
       ...
       ListFooterComponent={
         !showOnlyFavorites && isLoadingMore ? (
